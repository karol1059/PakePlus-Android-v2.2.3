...
function checkGameProgress() {
    ...
    if (versionInfo) {
        ...
        // 从localStorage获取存储的版本信息
        const storedVersionInfo = localStorage.getItem('versionInfo');
        let versionCounter = 200;
        let storedRevision = '';
        
        if (storedVersionInfo) {
            const parsedInfo = JSON.parse(storedVersionInfo);
            storedRevision = parsedInfo.revision;
            versionCounter = parsedInfo.counter;
        }
        
        // 如果代码被修改，增加版本号
        if (storedRevision !== codeRevision) {
            versionCounter++;
            // 存储新的版本信息
