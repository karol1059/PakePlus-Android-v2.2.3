...
const gameData = {
    ...
    weekDay: 4, // 0-6, 0 means Sunday, 4 means Thursday (June 1st)
    lunarDate: '农历四月廿八',
    // 农历内部计数
    lunarCounter: {
    ...
    movies: [
        { id: 1, name: '', type: 'Action', schedule: 30, tier: 1, releaseDay: 1, daysReleased: 0, inSchedule: true }, // Tier 1 (high priority)
        { id: 2, name: '', type: 'Comedy', schedule: 25, tier: 2, releaseDay: 1, daysReleased: 0, inSchedule: true }, // Tier 2 (medium priority)
        { id: 3, name: '', type: 'Romance', schedule: 20, tier: 3, releaseDay: 1, daysReleased: 0, inSchedule: true }, // Tier 3 (low priority)
        { id: 4, name: '', type: 'Sci-Fi', schedule: 25, tier: 1, releaseDay: 1, daysReleased: 0, inSchedule: true }  // Tier 1 (high priority)
    ],
    newMovieCounter: 0, // 新片计数器
    weekCounter: 0, // 周计数器
    products: [
        { id: 1, name: 'Popcorn', type: '基础商品', price: 15, stock: 100, todaySales: 0 },
        { id: 2, name: 'Cola', type: '基础商品', price: 10, stock: 80, todaySales: 0 },
        { id: 3, name: 'Merchandise', type: '衍生品', price: 50, stock: 30, todaySales: 0 }
    ],
    shopLevels: {
        d: 1,
        c: 0,
...
function loadGameProgress() {
    ...
    if (savedData) {
        ...
        // Ensure cooperation data exists
...
// Check game progress in local storage
function checkGameProgress() {
    ...
    if (hasSavedGame === 'true' && continueBtn) {
        // Load game data from local storage, update button text
        const savedData = localStorage.getItem('gameData');
        if (savedData) {
            const parsedData = JSON.parse(savedData);
            // Format game time
            const gameTime = `${parsedData.date.year}/${parsedData.date.month}/${parsedData.date.day}`;
            // Format assets
            const formattedAssets = formatAssets(parsedData.money);
            // Get cinema name
            let cinemaName = parsedData.cinemaName || 'Unnamed Cinema';
            // Update button HTML, use line break and small text to display
            const shopCount = (parsedData.shops ? parsedData.shops.length : 0) + 1; // Add 1 for main store
            ...
            const shopText = shopCount > 1 ? ` (${shopCount} stores)` : ''; // Only display when more than 1 store
            continueBtn.innerHTML = `Continue Game<br>${displayName}${shopText} | ${gameTime} | Assets: ¥${formattedAssets}`;
        }
    ...
    if (versionInfo) {
        ...
        versionInfo.textContent = `${year}-${month}-${day} ${hours}:${minutes}`;
    }
}

// 格式化资产金额，万以下用阿拉伯数字，万以上用中文单位
function formatAssets(amount) {
    // Handle floating point precision, round down first
    amount = Math.floor(amount);
    
    // English large number format
