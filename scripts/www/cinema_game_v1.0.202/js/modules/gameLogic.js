...
function openModal() {
    // 确保modalCount存在且为数字
    if (typeof gameData.modalCount !== 'number') {
        gameData.modalCount = 0;
    }
    
    gameData.modalCount++;
    console.log('打开弹窗后，modalCount:', gameData.modalCount);
...
function closeModal() {
    ...
    console.log('关闭弹窗后，modalCount:', gameData.modalCount);
    
    // 强制重置为0，确保游戏能够继续
    gameData.modalCount = 0;
    console.log('强制重置modalCount为0');
    
    // 恢复游戏
    gameData.timePaused = false;
    console.log('所有弹窗已关闭，恢复游戏');
...
export function startGameLoop() {
    ...
    // 根据游戏速度调整时间间隔
    const currentSpeed = window.gameSpeed || gameSpeed || 1;
    console.log('当前游戏速度:', currentSpeed);
    interval = Math.floor(interval / currentSpeed);
    console.log('游戏循环间隔:', interval, 'ms');
    
    // 开始新的游戏循环
    gameInterval = setInterval(simulateDay, interval);
    console.log('游戏循环已启动，interval ID:', gameInterval);
}
...
function simulateDay() {
    ...
    if (gameData.timePaused) {
        console.log('游戏暂停中，跳过simulateDay');
        // 即使暂停，也要更新UI以显示最新日期
        updateUI();
        return;
    }
    console.log('执行simulateDay，当前日期:', gameData.date.year, '年', gameData.date.month, '月', gameData.date.day, '日');
    console.log('timePaused:', gameData.timePaused);
