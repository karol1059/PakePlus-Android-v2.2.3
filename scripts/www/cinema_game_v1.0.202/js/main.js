...
function playAd() {
    ...
    // 尝试使用真实广告SDK
    let hasRealSDK = false;
    
    try {
        // 初始化广告（兼容不同版本的SDK）
        if (window.UMengAd) {
            console.log('使用UMengAd SDK');
            hasRealSDK = true;
            
            // 初始化SDK
            console.log('初始化UMengAd SDK');
            const initResult = window.UMengAd.init({
                appKey: '69be78386f259537c77f6405', // 友盟AppKey
                channel: 'cinema_game' // 渠道名称
            });
            console.log('SDK初始化结果:', initResult);
            
            // 播放激励视频广告
            console.log('请求播放激励视频广告');
            window.UMengAd.showRewardedVideo({
                adUnitId: '100007644', // 激励视频广告位ID
                onSuccess: function() {
                    console.log('广告播放成功');
            ...
            console.log('使用UM SDK');
            hasRealSDK = true;
            
            // 兼容旧版本SDK
            console.log('初始化UM SDK');
            const initResult = window.UM.init({
                appKey: '69be78386f259537c77f6405',
                channel: 'cinema_game'
            });
            console.log('SDK初始化结果:', initResult);
    ...
    // 如果没有真实SDK，使用模拟广告
    if (!hasRealSDK) {
        console.log('友盟广告SDK未加载或初始化失败，使用模拟广告');
        // 模拟广告播放过程
        setTimeout(() => {
            gameData.money += 10000; // 奖励10000元
            updateUI();
